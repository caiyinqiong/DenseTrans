...
class Vocabulary(StatefulUnit):
    ...
    def __init__(self, pad_value: str = '<PAD>', oov_value: str = '<OOV>', cls_value: str = '<CLS>'):
        """Vocabulary unit initializer."""
        super().__init__()
        self._pad = pad_value
        self._oov = oov_value
    ...
    def fit(self, tokens: list):
        ...
        self._context['term_index'][self._pad] = 0
        self._context['term_index'][self._oov] = 1
        self._context['term_index'][self._cls] = 2
        self._context['index_term'][0] = self._pad
        self._context['index_term'][1] = self._oov
        self._context['index_term'][2] = self._cls

        terms = set(tokens)
        for index, term in enumerate(terms):
            self._context['term_index'][term] = index + 3
            self._context['index_term'][index + 3] = term

    def transform(self, input_: list) -> list:
        """Transform a list of tokens to corresponding indices."""
        return [2] + [self._context['term_index'][token] for token in input_]
